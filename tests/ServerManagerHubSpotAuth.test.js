...
describe('ServerManager HubSpot OAuth initialization', () => {
  ...
  test('initializes HubSpot OAuth through the refresh flow and injects accessToken env', async () => {
    const startTokenRefresh = jest.fn().mockResolvedValue('hubspot-access-token');
    const serverContext = {
      serverID: 'hubspot-server',
      serverEntity: {
        serverId: 'hubspot-server',
        serverName: 'HubSpot',
    ...
    expect(startTokenRefresh.mock.calls[0][0]).toBeInstanceOf(HubSpotAuthStrategy);
