...
- Modern MCP `2026-07-28` validates a present `Origin` header before authentication to mitigate DNS rebinding. Its hostname must be the canonical public hostname, an exact `MCP_2026_ALLOWED_ORIGIN_HOSTNAMES` entry, or an exact loopback hostname; absent `Origin` remains valid for non-browser MCP clients. Malformed, `null`, or non-allowlisted origins receive HTTP 403.
